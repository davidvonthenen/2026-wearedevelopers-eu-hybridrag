#!/usr/bin/env python3
"""
Synchronous spaCy NER service for the BM25 RAG demo.

The service extracts named entities from supplied text, lowercases and
returns the entity names for keyword matching, and also returns label-aware
``entity_pairs`` for debugging or future retrieval experiments.
    ...
    Request (JSON):
        {
          ...
          "labels": ["PERSON", "ORG", "GPE"]   # optional spaCy label allow-list
        }
    Response (JSON):
        {
          "text": "...",
...
@lru_cache(maxsize=1)
def load_spacy():
    """Load the configured spaCy model once and reuse it for all requests."""
...
def _extract_entities(
    ...
    """Return lowercase entity text and spaCy labels for allowed entity types."""
    doc = nlp_obj(text)
    return [
...
def ner():
    """Handle a single synchronous NER request."""
    ...
    # Keep entity order stable while removing duplicate normalized names.
