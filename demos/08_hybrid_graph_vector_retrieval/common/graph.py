...
The graph stores structured recipe metadata explicitly.
That matters for allergy handling because "not allergic to sulfites" is not a
semantic-search problem. It is a graph/filtering problem, unless we enjoy letting
nearest-neighbor math make medical-adjacent decisions like a caffeinated raccoon.
...
def ensure_graph_schema(
    ...
    *,
    ...
    are weirdly inconsistent. Naturally, database DDL could not be allowed to be
    boring.
    """
...
def find_recipes_by_profile(
    ...
    *,
    ...
    ``content_query`` is applied as a case-insensitive substring match against
    ``Recipe.content``. ``content_notquery`` is applied as a case-insensitive
    exclusion against the same field. The content field is the human-readable
    recipe text extracted from the source URL during ingestion. Semantic search
    is excellent for "cozy soup with smoky beans"; exact content inclusion
    and exclusion belong in the graph, where vibes do not get a vote.
    """
