...
This script ingests recipe CSV rows and enriches them with human-readable text
fetched from each recipe URL when URL fetching is enabled.

* Neo4j stores structured recipe metadata, recipe chunks, labels, cautions, and
  provenance for deterministic graph filtering.
* OpenSearch stores dense vector chunks generated from recipe text plus repeated
  recipe metadata so each chunk carries identity and safety context.
* URL text extraction is attempted with bounded exponential backoff.
* Recipes with unavailable or unreadable pages are skipped when URL fetching is
  enabled so graph and vector stores do not contain partial records.

Graph retrieval should remain the primary mechanism for safety-sensitive filters
such as allergy cautions. Vector search is used for semantic recipe intent,
ingredient language, instruction detail, and fuzzy matching.
"""
...
class RecipeRecord:
    """Normalized representation of one recipe CSV row.

    Raw display labels are preserved for prompts and output, while normalized
    properties provide deterministic keyword values for graph and vector filters.
    """
...
def parse_json_list(value: Any) -> List[str]:
    ...
    try:
        ...
        # Defensive fallback for CSV columns that contain comma-separated values
...
def iter_recipe_records(csv_path: Path) -> Iterator[RecipeRecord]:
    """Yield normalized recipe records from the CSV file.
...
def _json_loads_maybe(text: str) -> Any:
    """Best-effort JSON parser for JSON-LD script content."""
    ...
    try:
        ...
        # Some sites wrap JSON-LD in comments or leave trailing markers. Strip
        # those common wrappers before giving up on the payload.
        text = text.strip("\ufeff \t\n\r")
...
def _iter_recipe_jsonld(value: Any) -> Iterator[Mapping[str, Any]]:
    """Recursively yield JSON-LD objects whose type includes Recipe."""
...
def _render_jsonld_recipe(recipe: Mapping[str, Any]) -> str:
    """Render common Schema.org Recipe fields into readable text."""
...
def extract_jsonld_recipe_text(html: str) -> str:
    """Extract recipe text from Schema.org JSON-LD blocks when available."""
...
def extract_visible_text(html: str) -> str:
    """Extract visible page text after removing script and layout-heavy tags."""
    ...
    # Prefer content-like tags over raw body text to reduce navigation, footer,
    # and unrelated page text before falling back to the whole body.
    lines: List[str] = []
...
def _retry_after_seconds(response: requests.Response, *, remaining_backoff: float) -> Optional[float]:
    ...
    try:
        ...
        # HTTP-date Retry-After values are valid, but this bounded ingest loop
        # only honors numeric delays.
        return None
    if delay < 0:
        return None
...
def _next_backoff_seconds(
    ...
    *,
    ...
    """Return the next retry delay without exceeding the total backoff budget."""
...
def _response_error(response: requests.Response) -> str:
    """Format an HTTP response failure for ingest logs and audit fields."""
...
def format_list(values: Iterable[str], *, none_text: str = "none listed") -> str:
    """Render a list of display values for metadata headers."""
...
def build_full_recipe_text(recipe: RecipeRecord, page_text: str) -> str:
    """Build the full recipe text stored in Neo4j as recipe evidence."""
...
def build_recipe_vector_chunks(recipe: RecipeRecord, page_text: str, *, chunk_size: int, chunk_overlap: int) -> List[str]:
    """Create vector chunks with recipe/allergy metadata available in every chunk."""
...
def build_chunk_payloads(recipe: RecipeRecord, chunk_texts: List[str]) -> List[Dict[str, Any]]:
    """Attach stable chunk IDs and chunk positions to vector/graph chunk text."""
...
def build_vector_document(
    ...
    *,
    ...
    """Build the OpenSearch document for one embedded recipe chunk."""
...
def recipe_to_graph_payload(
    ...
    *,
    ...
    """Build the Neo4j payload for one recipe and its fetched page text."""
...
def ingest_hybrid(
    ...
    *,
    ...
    """Run the end-to-end graph and vector ingestion workflow."""
...
def main(argv: Sequence[str] | None = None) -> None:
    """CLI entry point for recipe ingestion."""
