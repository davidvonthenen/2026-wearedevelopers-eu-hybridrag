...
The graph stores recipe records as nodes with structured metadata relationships.
This gives the query layer deterministic filters for cautions, diet labels,
health labels, cuisine, meal type, and dish type, while optional name/text
filters provide lightweight string matching.
...
def ensure_graph_schema(
    ...
    *,
    ...
    Full-text index creation is best-effort because Neo4j deployments can differ
    by version, configuration, and DDL privileges.
    """
...
def _run_schema(
    ...
    *,
    ...
    """Execute one schema statement and log failures without aborting ingestion."""
...
def ingest_recipe(
    ...
    *,
    ...
    Args:
        ...
        chunks: Graph chunk payloads stored as ``RecipeChunk`` nodes.
...
# --------------------------------------------------------------------------------------
# Query helpers for graph-first recipe filtering
# --------------------------------------------------------------------------------------


def _recipe_return_clause() -> str:
    """Return the shared Cypher projection used by graph query helpers."""
...
def find_recipes_by_profile(
    ...
    *,
    ...
    """Filter recipes with structured graph metadata and optional name/text matching.

    ``content_query`` and ``content_notquery`` are normalized to lowercase and
    applied by the Cypher predicate against ``r.name`` and ``r.text``. The
    returned records include ``r.content`` for display/audit, but this helper
    does not currently search ``r.content`` or ``RecipeChunk.text``.
    Structured filters are evaluated through typed graph relationships.
    """
    ...
    cypher = (
        ...
        "WHERE ($content_query = '' OR toLower(coalesce(r.name, '')) CONTAINS $content_query OR toLower(coalesce(r.text, '')) CONTAINS $content_query) "
        "AND ($content_notquery = '' OR NOT (toLower(coalesce(r.name, '')) CONTAINS $content_notquery OR toLower(coalesce(r.text, '')) CONTAINS $content_notquery)) "
        "AND NOT EXISTS { "
        "  MATCH (r)-[:HAS_CAUTION]->(a:AllergyWarning) "
        "  WHERE a.name IN $excluded_cautions "
        "} "
