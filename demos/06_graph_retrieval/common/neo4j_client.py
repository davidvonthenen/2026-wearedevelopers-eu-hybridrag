...
class MyNeo4j:
    ...
    def close(self) -> None:
        """Close the underlying Neo4j driver, suppressing close-time cleanup errors."""
...
def _auth_tuple(user: str, password: str) -> Optional[Tuple[str, str]]:
    """Return a Neo4j auth tuple, or ``None`` when no username is configured."""
...
def create_graph_client(settings: Optional[Settings] = None) -> MyNeo4j:
    """Create a Neo4j driver using the configured URI, credentials, and database."""
