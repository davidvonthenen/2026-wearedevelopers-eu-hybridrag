...
The recipe project uses Neo4j for graph truth grounding, OpenSearch for
semantic vector chunks, and an OpenAI-compatible chat endpoint for answer
generation. This module intentionally keeps ingest-facing recipe settings and
LLM-serving settings in one place so the CLI, REST agent, and local LLM server
share the same connection, model, and serving defaults.
"""
...
@dataclass
class Settings:
    """Connection, retrieval, embedding, and LLM settings for recipe Hybrid RAG."""

    # Neo4j (graph store)
    neo4j_uri: str = "bolt://127.0.0.1:7687"
    neo4j_user: str = "neo4j"
    neo4j_password: str = "neo4jneo4j1"
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        neo4j_database=os.getenv("NEO4J_DATABASE", Settings.neo4j_database),
